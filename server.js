...
server.get("/users", async (req, res) => {
    const getlist = await model_1.default.find({});
    res.send(getlist);
});
server.post("/users", async (req, res) => {
    try {
        if (req.body) {
            const Usrs = new model_1.default(req.body);
            await Usrs.save();
            res.send(Usrs);
        }
    }
...
server.put("/users/:id", async (req, res) => {
    if (req.body) {
        ...
        try {
            model_1.default.findOneAndUpdate({ _id: id }, req.body);
...
server.delete("/users/:id", async (req, res) => {
    const id = req.params.id;
    try {
        model_1.default.findOneAndDelete({ _id: id });
...
server.post("/login", async (req, res) => {
    ...
    if (req.body) {
        ...
        try {
            const result = await model_1.default.findOne({ uname, psw });
            if (!result) {
                // Skicka false.
